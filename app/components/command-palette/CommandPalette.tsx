  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh]">
      <button
        ...
        type="button"
      />
      <div className="relative w-full max-w-2xl bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl shadow-2xl overflow-hidden">
        <div className="flex items-center px-4 py-3 border-b border-[var(--card-border)]">
          <svg className="w-5 h-5 text-[var(--muted)] mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <input
            ...
            className="flex-1 bg-transparent text-[var(--foreground)] placeholder-[var(--muted)] outline-none text-lg"
          />
          <kbd className="px-2 py-1 text-xs bg-[var(--card-border)] text-[var(--muted)] rounded">ESC</kbd>
        </div>

        <div className="max-h-[60vh] overflow-y-auto">
          {Object.entries(groupedCommands).map(([category, cmds]) => (
            <div key={category}>
              <div className="px-4 py-2 text-xs font-semibold text-[var(--muted)] uppercase tracking-wider bg-[var(--card-bg)]/80">
                {category}
              </div>
              {cmds.map((cmd) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full px-4 py-3 flex items-center justify-between text-left transition-colors ${
                      isSelected ? 'bg-blue-600/20 text-[var(--foreground)]' : 'text-[var(--foreground)] hover:bg-[var(--card-border)]/20'
                    }`}
                  >
                    <span>{cmd.name}</span>
                    {cmd.shortcut && (
                      <div className="flex items-center gap-1">
                        {cmd.shortcut.split(' ').map((key, i) => (
                          <kbd key={i} className="px-1.5 py-0.5 text-xs bg-[var(--card-border)] text-[var(--muted)] rounded">
        ...
        <div className="px-4 py-2 text-xs text-[var(--muted)] border-t border-[var(--card-border)] flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-[var(--card-border)] rounded">↑↓</kbd> to navigate
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1.5 py-0.5 bg-[var(--card-border)] rounded">↵</kbd> to select
            </span>
          </div>
